# innerlink 产品需求（继承自 innerlink-core v0.5 PRD）

> 版本：v1.0.0（2026-06-23）单仓版；协议层 M1-M4 来自历史 innerlink-core v0.5（2026-06-18）
...
**`innerlink`（前身 innerlink-core，已合并到本仓）是一个纯 Go 实现的国密（SM2/SM3/SM4）端到端加密 P2P 通信核心库**。同 WiFi / 同 NAT 网段的两台电脑无需注册账号、无需联网，可以直接互发加密聊天和加密文件。
...
| 易嵌入 | 任何 Go 程序（CLI / 桌面 / 嵌入式 / 服务端）通过 `import "github.com/weishengsuptp/innerlink/pkg/node"` 即可构造一个长生命周期 `*node.Node`，调用 `New / Start / SendText / SendFile / ListPeers / SubscribeMessages / History / SetAlias` 等公开 API；底层 12 个 `internal/*` 包仍保持私有，不对外暴露 |
...
- **模块路径**：`github.com/weishengsuptp/innerlink`（v1.0.0 单仓后；v0.x 历史为 `github.com/weishengsuptp/innerlink`）
- **许可证**：Apache 2.0
