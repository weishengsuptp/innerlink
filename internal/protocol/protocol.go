...
const (
	...
	// merge. There is no incremental diff protocol 鈥?the
	...
	// scan 鈥?without this, two nodes on the same
...
// new entries (existing entries are kept as-is 鈥?local
...
// already scanned this session 鈥?both via the
...
// pong in response to a ping 鈥?sending one unsolicited is a
...
// 鈥?they will be serialized internally. (The underlying
...
func (c *Channel) Recv(ctx context.Context) (Envelope, error) {
	...
	// Re-check ctx after acquiring the lock 鈥?a cancellation
	...
	// current version only 鈥?future versions would need explicit
	// compat code.
