...
// the old Store and Open a new one 鈥?there is no Rotate()
...
type Store struct {
	...
	// (which is a graceful trade-off 鈥?we get 10x lower
...
// memory for the lifetime of the Store 鈥?this is safe as
// long as the process doesn't swap to disk or get its
// memory dumped. (v0.1 doesn't address either of those
// 鈥?M4+ would add mlock + GCM AEAD if it ever became a
// concern.)
//
// If chat.enc exists, Open does not validate it 鈥?the
...
// writing a few hundred bytes to the OS 鈥?we are not
