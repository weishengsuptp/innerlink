...
// "device" 鈥?i.e. this installation on this machine.
//
...
//
//       1. SM2 ECDH 鈥?shared secret derivation during handshake
//       2. SM2 signature VERIFICATION 鈥?confirming a message is from
...
// 16 bytes = 128 bits 鈥?collision-resistant for any realistic deployment
...
//
// The returned Identity holds a freshly generated private key 鈥?losing
...
// is the right primitive 鈥?anyone with a copy of D plus the
...
// same peer?" 鈥?used by the chat list, dedup logic, etc.
...
//
// Note: `id` itself isn't used for verification 鈥?only its type acts
...
// Windows, which is hidden behind the user's Library 鈥?not the place
...
//   - It's 40 bytes total 鈥?small enough to fit in a single UDP
...
// for this layer 鈥?see storage/ for any additional hardening).
...
//
// We do NOT auto-generate on missing file 鈥?callers that want
...
// (vs. an existing one being loaded) 鈥?useful for first-run UX hints
...
//
// Returns the concrete *sm2.PublicKey type 鈥?this is the one place in
...
// public key 鈥?typically a peer's. Lets us address-protocol messages
