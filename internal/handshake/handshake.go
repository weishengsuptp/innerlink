...
// panic on init 鈥?that would be a gmsm bug, not our bug.
package handshake
...
// identity package. Same shape as discovery.Device 鈥?a tiny,
// narrow interface.
...
func runHandshake(ctx context.Context, id Identity, conn *transport.Conn, initiator bool) (*Session, error) {
	...
	if initiator {
		...
		// B's "nonceA" is the nonce A sent in helloA 鈥?copy it out
...
func deriveSessionKey(id Identity, ephemeralPriv *sm2.PrivateKey, remotePub, remoteEphemeralPub, nonceA, nonceB []byte, isInitiator bool) ([]byte, error) {
	...
	// SM2 standard KeyAgreement per GM/T 0003-2012 搂6:
