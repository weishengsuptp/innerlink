...
type Receiver struct {
	...
	// 鈥?e.g. the receiver emits Accept as soon as it
	...
	// key 鈥?the most recent reply wins. (In v0.1 each
...
// silently dropped 鈥?the chat pump is a separate goroutine.
//
...
//
// The actual file-data path is: receive offer 鈫?maybe accept
// 鈫?stream chunks to disk 鈫?verify 鈫?send Done.
...
func (r *Receiver) Handle(ctx context.Context, env protocol.Envelope) {
	...
	// dispatcher pump 鈥?the dispatcher reads every envelope,
	...
	if env.Type == protocol.TypeFileAccept ||
		...
		if env.Type == protocol.TypeFileDone {
			...
			_ = json.Unmarshal(env.Payload, &d)
			if d.OK {
				_ = d.FileID // success log only 鈥?keep noise down
			}
		}
		return
	}
...
// for this to work 鈥?otherwise the envelope never reaches
// us and we deadlock.
//
...
// Accept (with the empty "have" list 鈥?v0.1 has no resume
...
// is the last chunk 鈥?if so, verify the full file hash and
// send Done.
func (r *Receiver) handleChunk(ctx context.Context, env protocol.Envelope) {
	...
	if !ok {
		// Unknown fileID 鈥?probably an old chunk arriving
