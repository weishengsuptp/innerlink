// Package node is the public runtime API of innerlink.
//
// innerlink (formerly innerlink-core, merged into the
// innerlink monorepo at v1.0.0) is a LAN P2P encrypted
// chat + file transfer library.
...
// never an error 鈥?the announcer treats this as informational only.
...
//
//  1. Literal "ip:port" 鈥?e.g. "dial 192.168.2.5:4748".
//     Returned unchanged. Detected by the presence
//     of a colon (host:port syntax) OR by parseable
//     dotted-quad IP form.
//
//  2. Alias name 鈥?e.g. "dial alice". Resolved via
...
// DialAddr is the public entry for "dial <ref>" 鈥?// connect directly to a peer by alias, peer-id, or
